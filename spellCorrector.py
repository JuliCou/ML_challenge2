'''
    Source: https://github.com/makcedward/nlp/blob/ee6ae825f7914df24ec4b21da00e24c6f0099152/aion/util/spell_check.py#L66
'''
...
class SymSpell(SpellCheck):
    ...
    def build_vocab(self, dictionary, file_dir, file_name, verbose=0):
        ...
        with io.open(file_dir + file_name, "w") as text_file:
            for token, count in dictionary.items():
                try:
                    text_file.write(str(token) + ' ' + str(count))
                    text_file.write('\n')
                except:
                    print(token)
    ...
    def best_correction(self, sentence):
        results = self.model.word_segmentation(sentence)
        return [{"word" : results.corrected_string, "distance" : results.distance_sum}]
